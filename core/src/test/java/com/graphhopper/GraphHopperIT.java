...
public class GraphHopperIT
{
    ...
    public void testAlternativeRoutes()
    {
        ...
        assertEquals(2, rsp.getPathWrappers().size());

        assertEquals(1310, rsp.getPathWrappers().get(0).getTime() / 1000);
        assertEquals(1356, rsp.getPathWrappers().get(1).getTime() / 1000);

        req.getHints().put("alternative_route.max_paths", "3");
        req.getHints().put("alternative_route.min_plateau_factor", "0.1");
        rsp = hopper.route(req);
        assertFalse(rsp.hasErrors());
        assertEquals(3, rsp.getPathWrappers().size());

        assertEquals(1310, rsp.getPathWrappers().get(0).getTime() / 1000);
        assertEquals(1356, rsp.getPathWrappers().get(1).getTime() / 1000);
        assertEquals(1416, rsp.getPathWrappers().get(2).getTime() / 1000);
    }
    ...
    public void testAlternativeRoutesBikeAndCar()
    {
        ...
        assertEquals(3, rsp.getPathWrappers().size());
        // via ramsenthal
        assertEquals(2864, rsp.getPathWrappers().get(0).getTime() / 1000);
        // via unterwaiz
        assertEquals(3320, rsp.getPathWrappers().get(1).getTime() / 1000);
        // via eselslohe -> theta; BTW: here decreasing time as priority influences time order
        assertEquals(3094, rsp.getPathWrappers().get(2).getTime() / 1000);
        ...
        assertEquals(870, rsp.getPathWrappers().get(0).getTime() / 1000);
        // via ramsenthal -> lerchenhof
        assertEquals(913, rsp.getPathWrappers().get(1).getTime() / 1000);
        // via neudrossenfeld
        assertEquals(958, rsp.getPathWrappers().get(2).getTime() / 1000);
    }
